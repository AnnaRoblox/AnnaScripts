...
CMDs[#CMDs + 1] = {NAME = 'ghost', DESC = 'fly but noclip'}
CMDs[#CMDs + 1] = {NAME = 'unghost', DESC = 'disables ghost'}
...
CMDs[#CMDs + 1] = {NAME = 'bringua / brua', DESC = 'Teleports unanchored parts'}
CMDs[#CMDs + 1] = {NAME = 'animsunanchored / freezeua', DESC = 'Freezes unanchored parts'}
CMDs[#CMDs + 1] = {NAME = 'deleteunanchored / deleteua / voidua', DESC = 'Tps unanchored parts to the Void'}
CMDs[#CMDs + 1] = {NAME = 'followunanchored / followua', DESC = 'makes unanchored parts follow a player'}
CMDs[#CMDs + 1] = {NAME = 'highlightua / hlua', DESC = 'highlights parts'}
CMDs[#CMDs + 1] = {NAME = 'unhighlightua / unhlua', DESC = 'disabled highlightua'}
...
CMDs[#CMDs + 1] = {NAME = 'unheadsit [player]', DESC = 'Sit on a players head'}
CMDs[#CMDs + 1] = {NAME = 'suck [player]', DESC = 'suck a player'}
CMDs[#CMDs + 1] = {NAME = 'unsuck [player]', DESC = 'unsuck a player'}
CMDs[#CMDs + 1] = {NAME = 'facesit [player]', DESC = 'Sit on a players face'}
CMDs[#CMDs + 1] = {NAME = 'unfacesit [player]', DESC = 'Sit on a players face'}
...
CMDs[#CMDs + 1] = {NAME = 'schat', DESC = 'silent chat'}
...
CMDs[#CMDs + 1] = {NAME = 'pcollision', DESC = 'enables collision'}
...
CMDs[#CMDs + 1] = {NAME = 'givetool / gt', DESC = 'Gives all the tools you\'re holding to [player] using the drop method.'}
CMDs[#CMDs + 1] = {NAME = 'givetools / gts', DESC = 'Gives all the tools to [player] using the drop method.'}
...
addcmd('ghost',{},function(args, speaker)
	execCmd('fly')
        execCmd('noclip')
	end)
addcmd('unghost',{},function(args, speaker)
	execCmd('unfly')
        execCmd('clip')
	end)
	...
	for i,v in pairs(players)do
		...
		headSit = RunService.Heartbeat:Connect(function()
			if Players:FindFirstChild(Players[v].Name) and Players[v].Character ~= nil and getRoot(Players[v].Character) and getRoot(speaker.Character) and speaker.Character:FindFirstChildOfClass('Humanoid').Sit == true then
				getRoot(speaker.Character).CFrame = getRoot(Players[v].Character).CFrame * CFrame.Angles(0,math.rad(180),0)* CFrame.new(0,1.6,0.8)
			else
				headSit:Disconnect()
			end
		end)
...
addcmd('unfacesit',{},function(args, speaker)
	facesit:Disconnect()
			
		end)
addcmd('facesit',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	if faceSit then faceSit:Disconnect() end
	for i,v in pairs(players)do
		speaker.Character:FindFirstChildOfClass('Humanoid').Sit = true
		facesit = RunService.Heartbeat:Connect(function()
			if Players:FindFirstChild(Players[v].Name) and Players[v].Character ~= nil and getRoot(Players[v].Character) and getRoot(speaker.Character) and speaker.Character:FindFirstChildOfClass('Humanoid').Sit == true then
				getRoot(speaker.Character).CFrame = getRoot(Players[v].Character).CFrame * CFrame.Angles(0,math.rad(180),0)* CFrame.new(0,1.6,0.8)
			else
				facesit:Disconnect()
...
addcmd("bang", {"rape"}, function(args, speaker)
	...
	if args[1] then
		...
		for _, v in pairs(players) do
			...
			local bangOffet = CFrame.new(0, 0, -0.5)
			bangLoop = RunService.Stepped:Connect(function()
				pcall(function()
					local otherRoot = getTorso(Players[bangplr].Character)
					getRoot(speaker.Character).CFrame = otherRoot.CFrame * bangOffet * CFrame.Angles(0,math.rad(180),0)* CFrame.new(0,1.6,0.4)
				end)
			end)
		end
	end
...
addcmd("pcollision", {}, function(args, speaker)
    if pcollision then
        pcollision:Disconnect()
        pcollision = nil
    end
    pcollision = RunService.Stepped:Connect(function()
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= speaker and player.Character then
                for _, v in pairs(player.Character:GetDescendants()) do
                    if v:IsA("BasePart") then
...
local followFolder = nil
local followConnection = nil

local followFolder = nil
local followConnection = nil
local originalCollisions = {} -- Table to store old CanCollide states

addcmd('followua', {'fua'}, function(args, speaker)
    -- Clean up previous session if it exists
    if followConnection then followConnection:Disconnect() end
    if followFolder then followFolder:Destroy() end
    originalCollisions = {} -- Reset the collision tracker

    -- Determine Target
    local targetName = args[1]
    local targetPlayer = speaker
    ...
    followFolder = Instance.new("Folder")
    followFolder.Name = "FollowUAFolder"
    followFolder.Parent = workspace

    local centerAttachment = Instance.new("Attachment")
    centerAttachment.Name = "CenterAttr"
    centerAttachment.Parent = root
    ...
    local function setupPhysics(v)
        if v:IsA("BasePart") and v.Anchored == false then
            local isBad = false
            for i = 1, #badnames do
                if v.Name == badnames[i] then isBad = true break end
            end
            if v:IsDescendantOf(targetPlayer.Character) then isBad = true end

            if not isBad then
                -- Store the original collision state before changing it
                originalCollisions[v] = v.CanCollide
                v.CanCollide = false

                -- Create attachment on the part
                local partAttr = Instance.new("Attachment")
                partAttr.Parent = v
                partAttr.Name = "FollowUA_Attr"

                -- Align Position
                local ap = Instance.new("AlignPosition")
                ap.Attachment0 = partAttr
                ap.Attachment1 = centerAttachment
                ...
                ao.Attachment0 = partAttr
                ao.Attachment1 = centerAttachment
                ao.MaxTorque = math.huge
                ao.Responsiveness = 30
                ao.Parent = v
                
                -- Cleanup tags
                partAttr.Name = "FollowCleanup"
                ap.Name = "FollowCleanup"
                ao.Name = "FollowCleanup"
            end
        end
    end
    ...
    followConnection = workspace.DescendantAdded:Connect(setupPhysics)
end)

addcmd('unfollowua', {'unfua', 'stopua'}, function(args, speaker)
    ...
    -- Restore Collisions and cleanup physics
    for _, v in pairs(workspace:GetDescendants()) do
        -- Check if we have a stored collision state for this part
        if originalCollisions[v] ~= nil then
            v.CanCollide = originalCollisions[v]
        end

        -- Delete our physics objects
        if v.Name == "FollowCleanup" or v.Name == "CenterAttr" or v.Name == "FollowUA_Attr" then
            v:Destroy()
        end
    end

    -- Final folder and table cleanup
    if followFolder then
        followFolder:Destroy()
        followFolder = nil
    end
    originalCollisions = {}
end)
addcmd('bringua', {'brua', 'getua'}, function(args, speaker)
    ...
    -- Gather valid parts first to avoid issues with workspace changing during loop
    local partsToBring = {}
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("BasePart") and v.Anchored == false then
            local isBad = false
            ...
            if v:IsDescendantOf(speaker.Character) then isBad = true end

            if not isBad then
                table.insert(partsToBring, v)
            end
        end
    end

    -- The Bring Process
    for _, part in pairs(partsToBring) do
        -- 1. Teleport to the part to gain ownership
        root.CFrame = part.CFrame
        
        -- 2. Brief wait for the engine to register proximity
        task.wait(0.05) 
        
        -- 3. Bring the part to the original position
        part.CFrame = originalPos
        
        -- 4. Teleport back to original position
        root.CFrame = originalPos
        
        -- Small gap to prevent the game from freezing
        task.wait(0.01)
    end
    
    -- Ensure speaker is back at the start
    root.CFrame = originalPos
end)
local activeHighlights = {}

addcmd('highlightua', {'hlua'}, function(args, speaker)
    ...
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("BasePart") and v.Anchored == false then
            ...
            if speaker.Character and v:IsDescendantOf(speaker.Character) then isBad = true end

            if not isBad then
                -- Ownership Check logic
                local isOwned = (v.ReceiveAge == 0)
                
                -- If user specified "true/t", skip parts we don't own
                if onlyOwned and not isOwned then
                    continue 
                end

                -- Create Highlight
                local hl = Instance.new("Highlight")
                hl.Name = "UA_Highlight"
                hl.Adornee = v
                
                -- Visual cues: Green for owned, Red for unowned
                if isOwned then
                ...
                hl.Parent = v -- Putting it inside the part ensures it cleans up if the part is deleted
...
addcmd('unhighlightua', {'unhlua'}, function(args, speaker)
    for _, v in pairs(workspace:GetDescendants()) do
        if v.Name == "UA_Highlight" and v:IsA("Highlight") then
            v:Destroy()
        end
    end
    activeHighlights = {}
end)
addcmd('deleteunanchored',{'deleteua','voidua'},function(args, speaker)
    local badnames = {
        "Head",
        "UpperTorso",
        "LowerTorso",
    ...
    local voidHeight = workspace.FallenPartsDestroyHeight - 10

    local function VOIDNOOB(v)
        -- Check if it's a part and unanchored
        if v:IsA("BasePart") and v.Anchored == false then
            local BADD = false
            
            -- Don't delete character parts
            for i = 1,#badnames do
                if v.Name == badnames[i] then
                    BADD = true
                end
            ...
            -- Don't delete our own character
            if speaker.Character and v:IsDescendantOf(speaker.Character) then
                BADD = true
            end

            -- If it's not a protected part, send it to the void
            if BADD == false then
                v.CFrame = CFrame.new(0, voidHeight, 0)
            end
        end
    end

    -- Run the sweep through the workspace
    for i,v in pairs(workspace:GetDescendants()) do
        VOIDNOOB(v)
    end
end)
